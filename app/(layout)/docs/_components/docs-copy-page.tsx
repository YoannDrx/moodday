...
function getPromptUrl(baseURL: string, prompt: string) {
...
export function DocsCopyPage({ page, url }: { page: string; url: string }) {
  ...
  const { t } = useI18n();
  const prompt = t("docs.copy.prompt", {
  ...
  const menuItems = {
    ...
    v0: () => (
      ...
      >
        ...
        <span className="-translate-x-[2px]">{t("docs.copy.openV0")}</span>
      </a>
    ),
    chatgpt: () => (
      <a
