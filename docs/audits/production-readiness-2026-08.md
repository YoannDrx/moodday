...
- [ ] Bases, Stripe, OpenAI, Resend, Blob et VAPID séparés par environnement.
...
- La pull request #5 est fusionnable. GitGuardian, Vercel, lint, typage, build,
  ...
  le commit `d929914`.
- Redis et Upstash ont été retirés de l'architecture : les limites de débit
  utilisent déjà une écriture atomique PostgreSQL et la seule préférence UI
  restante est stockée localement dans le navigateur. Aucun cache, rappel,
  paiement, traitement IA ou parcours métier ne dépendait de Redis.
- L'éditeur public et le responsable du traitement sont identifiés comme
  Yodev, entrepreneur individuel, SIREN 803 272 590. L'adresse n'est pas
  publiée car la fiche officielle est non diffusible.
